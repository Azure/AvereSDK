...
| `--data-disk-type` *volume_type* | Type of storage volumes to create for the vFXT cluster cache. Azure supports only one disk type (premium LRS) and does not use this option. |
...
| `--image-id` *image_ID_or_URI* | Optionally, use this parameter to specify an image instead of using the default image when creating the cluster. Consult support for guidance before using this advanced option. <br/>The image ID or URL should match what your cloud provider uses. <br/> • Azure URN example: microsoft-avere:vfxt-preview:avere-vfxt-node:latest |
...
| `--azurecontainer` *blob_container_name* | Name of an existing, empty cloud storage container to use as the core filer. <br/> • For Azure, use `--azurecontainer` to specify a blob container |
