...
Additional options are explained in the [vfxt.py Command Syntax and Options](syntax.md) document and in the platform-specific documents for [Azure](azure_options.md). 
...
* Authentication credentials for accessing the cloud API. The syntax is different depending on the cloud provider and authentication method you are using; read the setup section for [Azure](azure_reference.md) for details. 
...
* Azure uses the instance name, which is the text string you used to name the cluster followed by the node numbers, -01, -02, etc.
